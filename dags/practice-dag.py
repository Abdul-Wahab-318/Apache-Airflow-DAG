...
def extract(**kwargs):
    ...
    ti.log.info("html response length: %s" ,len(response.text))

def transform(**kwargs):
    
    ti = kwargs['ti']
    html_reponse = ti.xcom_pull(key="html_response" , task_ids="extract_task" )
    ti.log.info("html response length : %s" , len(html_reponse))
    
    html = BeautifulSoup(html_reponse , features='lxml')
    price_div = html.find('div', class_='t4s-product-price')
